    ...
    for key, value in results_by_gen.items():
        ...
        for i, turn in enumerate(value):
            ...
            if speedup_ >= speedup_threahold:
                ...
                speedup_reward = 0
            step_reward = correctness_reward + speedup_reward
            if previous_max != -1 and (
                step_reward > previous_max
                or speedup_ > previous_max_speedup >= speedup_threahold
            ):  # reward incremental speed up
...
def grpo_compute_rewards_v3(
    ...
    for key, value in results_by_gen.items():
        ...
        for i, turn in enumerate(value):
            ...
            step_reward = correctness_reward + speedup_reward
            if i > 0 and (
                step_reward > previous_max or speedup_ > previous_max_speedup * 1.2
            ):  # reward incremental speed up
                previous_max = max(previous_max, step_reward)
                previous_max_speedup = max(previous_max_speedup, speedup_)
                step_reward += improvement_bonus
...
def grpo_compute_rewards_v4(
    ...
    for key, value in results_by_gen.items():
        ...
        for i, turn in enumerate(value):
            ...
            step_reward = correctness_reward + speedup_reward
            if i > 0 and (
                previous_max_speedup >= speedup_threahold
                and speedup_ > previous_max_speedup * 1.5
            ):  # reward incremental speed up
                step_reward += improvement_bonus
            previous_max_speedup = max(previous_max_speedup, speedup_)
            trajectory_reward = step_reward
...
def grpo_compute_rewards_v5(
    ...
    for key, value in results_by_gen.items():
        ...
        for i, turn in enumerate(value):
            ...
            if (
                i > 0
                and (speedup_ > previous_max_speedup >= speedup_threahold)
                and had_improvement is False
            ):  # reward incremental speed up
                step_reward += improvement_bonus
...
def speedup_alpha(ref_dict, test_dict, speedup_threshold=1.3, alpha=0.05):
    ...
    df = (s_test_sq + speedup_threshold**2 * s_ref_sq) ** 2 / (
        s_test_sq**2 / (n_test - 1)
        + (speedup_threshold**2 * s_ref_sq) ** 2 / (n_ref - 1)
    )
...
if __name__ == "__main__":
    ...
    groups = grpo_compute_rewards_v5(
    ...
    for key, value in group_datasets.items():
        ...
        print(
            "    => len(vllm_completion_log_probs): ",
            [
                f'{len(gen["vllm_completion_log_probs"])}'
                for gen in value
                if gen["vllm_completion_log_probs"]
            ],
        )
        print(
            "    => len(vllm_input_ids): ",
            [
                f'{len(gen["vllm_input_ids"])}'
                for gen in value
                if "vllm_input_ids" in gen
            ],
        )
        print(
            "    => len(vllm_attention_mask): ",
            [
                f'{len(gen["vllm_attention_mask"])}'
                for gen in value
                if "vllm_attention_mask" in gen
            ],
        )
        print(
            "    => len(logp_server_prompt_ids): ",
            [
                f'{len(gen["logp_server_prompt_ids"])}'
                for gen in value
                if "logp_server_prompt_ids" in gen
            ],
        )
        print(
            "    => len(logp_server_completion_ids): ",
            [
                f'{len(gen["logp_server_completion_ids"])}'
                for gen in value
                if "logp_server_completion_ids" in gen
            ],
        )
        print(
        ...
        print(
            "    => len(logp_server_logps): ",
            [
                f'{len(gen["logp_server_logps"])}'
                for gen in value
                if "logp_server_logps" in gen
            ],
        )
        print(
            "    => len(logp_server_attention_mask): ",
            [
                f'{len(gen["logp_server_attention_mask"])}'
                for gen in value
                if "logp_server_attention_mask" in gen
            ],
        )

    # Test case 1: Clear speedup
    ref = {"mean": 100, "std": 10, "min": 80, "max": 120, "num_trials": 10}
    ...
    ref2 = {"mean": 100, "std": 10, "min": 80, "max": 120, "num_trials": 10}
    ...
    print(
        f"Maximum claimable speedup threshold: {max_result1['max_speedup_threshold']:.3f}x"
    )
    ...
    verify1 = speedup_alpha(
        ref, test, speedup_threshold=max_result1["max_speedup_threshold"], alpha=0.05
    )
    ...
    print(
        f"Maximum claimable speedup threshold: {max_result2['max_speedup_threshold']:.3f}x"
    )
    ...
    print(
        f"Maximum claimable speedup threshold: {max_result3['max_speedup_threshold']:.3f}x"
    )
