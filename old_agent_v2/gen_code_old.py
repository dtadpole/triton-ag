...
def _prep_working_directory(args):
    ...
    while os.path.exists(
        os.path.join(args.dir, f"_code_gen_{args.model}_{next_sequence_id:03d}")
    ):
        next_sequence_id += 1
    folder_name = os.path.join(
        args.dir, f"_code_gen_{args.model}_{next_sequence_id:03d}"
    )
    ...
    for file in os.listdir(args.dir):
        if (
            file == "prompt.txt"
            or file.endswith(".py")
            or file.endswith(".stderr")
            or file.endswith(".stdout")
            or file.endswith(".success")
            or file.endswith(".failed")
...
async def main(args, model_settings):
    ...
    async def establish_plan(ctx: RunContext[ManagerState]) -> CodePlan:
        ...
        return await agent_planner.run(
            ctx.deps.task, usage=ctx.usage, model_settings=model_settings
        )
    ...
    async def generate_code(ctx: RunContext[ManagerState], plan: CodePlan) -> CodeFile:
        ...
        files = [
            f
            for f in os.listdir(ctx.deps.wd)
            if f.endswith(".py") and not f.startswith("_")
        ]
        for file in files:
            with open(os.path.join(ctx.deps.wd, file), "r") as f:
                prompt.append({"type": "file", "filename": file, "file": f.read()})

        code_file = await agent_coder.run(
            json.dumps(prompt, indent=2), usage=ctx.usage, model_settings=model_settings
        )
    ...
    result = await agent_manager.run(
        task, deps=manager_state, model_settings=model_settings
    )
    ...
    if "env" in provider_settings:
        for env_var in provider_settings["env"]:
            if "value" in env_var:
                ...
                raise ValueError(
                    f"Invalid env variable: {env_var} for [{args.provider}]"
                )
    ...
    for model in provider_settings["models"]:
        if model["name"] == args.model:
            if "settings" not in model:
                raise ValueError(
                    f"Invalid model: {args.model} for [{args.provider}], missing 'settings'"
                )
