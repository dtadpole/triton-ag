#!/usr/bin/env python3
"""
Test to verify TDC checkpoint functionality by simulating distributed mode
"""
...
def test_tdc_checkpoint_verification():
    """Test TDC checkpoint functionality by forcing distributed mode"""
    logger.info("🧪 Testing TDC checkpoint verification...")
    ...
    import time
    unique_name = f"test_tdc_{int(time.time())}"
    ...
    # Now test TDC checkpoint save/load
    with tempfile.TemporaryDirectory() as temp_dir:
        checkpoint_path = Path(temp_dir) / "test_tdc_checkpoint"
        checkpoint_path.mkdir(exist_ok=True)
        
        logger.info(f"💾 Saving TDC checkpoint to: {checkpoint_path}")
        ...
        logger.info("📂 Loading TDC checkpoint...")
        ...
        logger.info(f"📋 TDC State verification results:")
        ...
        if all_match:
            logger.info("🎉 All TDC checkpoint states match perfectly!")
        else:
            logger.error("❌ Some TDC checkpoint states don't match!")
...
if __name__ == "__main__":
    try:
        # Test TDC checkpoint verification
        success = test_tdc_checkpoint_verification()
        
        if success:
            logger.info("🎉 TDC checkpoint verification test passed!")
            sys.exit(0)
        else:
            logger.error("❌ TDC checkpoint verification test failed!")
            sys.exit(1)
