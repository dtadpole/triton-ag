#!/usr/bin/env python3
"""
Simple test to verify that saved checkpoint state exactly matches loaded state
"""
...
def test_simple_checkpoint_verification():
    """Test that saved checkpoint state exactly matches loaded state"""
    logger.info("🧪 Testing simple checkpoint state verification...")
    ...
    # Create trainer
    trainer = EngineFSDP("test_simple", config, inference_mode=False)
...
if __name__ == "__main__":
    try:
        ...
        success = test_simple_checkpoint_verification()
        
        if success:
            logger.info("🎉 Checkpoint verification test passed!")
            sys.exit(0)
        else:
            logger.error("❌ Checkpoint verification test failed!")
            sys.exit(1)
