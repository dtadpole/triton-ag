...
def flash_moe_kernel_fwd(
    ...
    """Kernel for computing the multi-head moe
    Z = X @ W1, H = f(Z), O = H @ W2.
    - X has shape (B, D)
    - W1 has shape (D, E)
    - W2 has shape (E, D)
...
def flash_moe_triton_fwd(x, w1, w2, activation=""):
    # Check constraints.
...
def mhmoe_torch_fwd(x, w1, w2, activation=""):
    z = torch.bmm(x, w1)
    if activation == "leaky_relu":
...
def _flash_moe_kernel_bwd_dx(
    ...
    """Kernel for computing the moe_bwd_dx
    Z = X @ W1, H = f(Z), O = H @ W2
    - X has shape (B, D)
    - W1 has shape (D, E)
    - W2 has shape (E, D)
...
@triton.jit
def _flash_moe_kernel_bwd_dw1w2(
    dw1, dw2,
    pid_h, pid_e,
    x_ptr, w1_ptr, w2_ptr, o_ptr, dx_ptr, dw1_ptr, dw2_ptr, do_ptr,
    H, B, D: tl.constexpr, E,
    ...
    """Kernel for computing the moe_bwd_dw1w2
    Z = X @ W1, H = f(Z), O = H @ W2
    - X has shape (B, D)
    - W1 has shape (D, E)
    - W2 has shape (E, D)
...
def flash_moe_kernel_bwd2(
    ...
    if idx >= batch_groups_e:
        ...
        dw1, dw2 = _flash_moe_kernel_bwd_dw1w2(
            dw1, dw2,
            pid_h, pid_e,
            x_ptr, w1_ptr, w2_ptr, o_ptr, dx_ptr, dw1_ptr, dw2_ptr, do_ptr,
            H, B, D, E,
...
def flash_moe_triton_bwd2(x, w1, w2, o, do, activation=""):
    # Check constraints.
...
def mhmoe_torch_bwd2(x, w1, w2, o, do, activation="") -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if activation == "leaky_relu":
...
def unit_test_fwd():
    ...
    triton_output = flash_moe_triton_fwd(x, w1, w2, activation=activation)
    torch_output = mhmoe_torch_fwd(x, w1, w2, activation=activation)
...
def unit_test_bwd2():
    ...
    triton_output = flash_moe_triton_bwd2(x, w1, w2, o, do, activation=activation)
    # triton_output = mhmoe_torch_bwd_raw(x, w1, w2, o, do, activation=activation)
    torch_output = mhmoe_torch_bwd2(x, w1, w2, o, do, activation=activation)
    ...
    if torch.allclose(triton_output[1], torch_output[1], atol=eplison, rtol=eplison):
        ...
        print("❌ [Backward] Triton and Torch differ")
...
if __name__ == '__main__':
    ...
    # mhmoe_torch(x, w1, w2, activation="leaky_relu")

    # o = flash_moe_triton_fwd(x, w1, w2, activation="leaky_relu")
    # print(o.shape)
