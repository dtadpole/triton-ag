...
2. **Losses with log operations (KLDiv, CrossEntropy)**: Tier 2 -- Fuse log into the kernel to save one memory pass. Best opportunity for improvement (~1.1x).
3. **Simple losses (MSE, Huber, Hinge, Triplet)**: Tier 2/3-4 -- Two-stage map-reduce (per-block partial + final reduction). ~1.05x ceiling.
...
6. **Multi-row per program**: Tier 3-4 -- 2 rows can reduce scheduling overhead. 4+ rows cause register pressure.
