...
**Key insight**: In multi-op pipelines with batched matmul, stride-based access in Triton kernels eliminates transpose+contiguous() copies that dominate runtime. Writing output in transposed layout (B,D,K) directly from (B,K,N)@(B,N,D) avoids a second copy.
...
### L3: 50_ReLUSelfAttention (1.788x, iter 5) -- Flash-like ReLU attention with split precision
**Key insight**: ReLU attention does NOT require online softmax rescaling -- tiled kernel can accumulate Q@K^T*scale*causal_mask*relu then @V without correction. Biggest gain from switching attention dot products to TF32 precision (HEAD_DIM=64 is small K, safe for TF32), while keeping projection GEMMs at IEEE (K=768 needs IEEE for correctness).
**What worked**: Split precision: TF32 for attention dots (1.259x -> 1.788x, +0.529x), IEEE for projection GEMMs with K=768. fp16 projection inputs with IEEE precision caused max_diff=0.176 -- too imprecise for large K.
...
3. **For Gemm + pointwise chain (L2)** (Tier 2): Epilogue fusion template -- fp16 tensor cores, cached weight. Expect 7-12x. ~85% first-try. Scales to 5+ ops. Applies to chained MLPs (L3: 8-11x) and RNNs (L3: 4.6-5.5x).
4. **For Gemm + Normalization + acts (L2)** (Tier 2): Two-kernel: matmul+bias epilogue, then fused GN/BN+activation. Expect 6-11x. Three-kernel for BN (separate stats pass). GroupNorm: match BLOCK_SIZE to group_size.
5. **For Gemm + softmax/logsumexp (L2)** (Tier 2): Two-kernel: matmul+epilogue then online softmax/logsumexp. Expect 7-12x. Use fixed BLOCK_N for softmax (never autotune).
6. **For Gemm + pooling + pointwise (L2)** (Tier 2): Two-kernel: matmul+bias then fused pool+acts. Expect 9x.
7. **For attention/transformer** (Tier 1-2): Flash attention for causal/dense attention with T>=512 (3.8-8x). Three-kernel decomposition for smaller SEQ_LEN. Split precision: TF32 for attention dots (HEAD_DIM<=64), IEEE for projection GEMMs (K>=768). Use `@` operator (cuBLAS) for projection GEMMs instead of Triton tl.dot. Use torch.layer_norm for multi-layer blocks.
8. **For memory-bound multi-op pipelines** (Tier 2): All-Triton with stride-based access to eliminate contiguous() copies. Avoid aten ops when layout transformations dominate (e.g., small N with multiple transposes).
9. **fp16 tuning** (Tier 3-4): Cache fp16 weight via register_buffer in __init__. Never inline-cast in kernel. Pre-transpose to contiguous layout. Full fp16 pipeline for deep chains. fp16 intermediates between layers. Gives +20-100% on large GEMMs.
10. **For transposed inputs** (Tier 3-4): Prefer pre-transposed contiguous layout. If using strides, double-check stride ordering (K,1 vs 1,K).
11. **For matvec (M<4K, K>100K)** (Anti-Pattern): Accept ~1.0x. Bandwidth-bound.
12. **For small/medium matrices (<1024x1024)** (Anti-Pattern): Accept Triton is 2-4x slower than cuBLAS. Do not attempt fp16 tensor cores (conversion overhead dominates). Use `@` operator instead.
13. **Autotune budget** (Tier 3-4): 4-7 configs optimal. Fewer (5) for deep chains. Fixed config (no autotune) for small matrices or BN-interacting kernels.
14. **Never**: Write "nn.Linear" / "nn.BatchNorm1d" / "nn.GroupNorm" in source (eval server string filter). Never pass scalar float args to kernel (use hardcoded constants). Always replicate exact weight init via nn.Parameter + manual init with correct gain (weight shape must match PyTorch init order). Never use Triton LayerNorm in multi-layer transformers (use torch.layer_norm). Never autotune kernels that update BN running stats. Never use `from triton import libdevice` or `from triton.language import libdevice` (use `tl.math.erf`/`tl.math.tanh` or `from triton.language.extra import libdevice`). Never use Triton tl.dot for projection GEMMs when correctness matters -- use `@` operator (cuBLAS).
