...
## Decision Framework for Pooling Tasks

1. **MaxPool (any dim)**: Flat spatial + per-element max over kernel window. 2D grid (spatial_blocks, batch*channels). Expect 1.4-3.0x. Usually first-try success.
2. **AvgPool3D (small kernel)**: Direct flat indexing with unrolled window. ~2.0x.
3. **AvgPool1D/2D (stride=1)**: Interior/boundary split. ~1.07x ceiling (cuDNN already efficient).
4. **Large pool windows (11x11+)**: Do NOT use constexpr for kernel_size (compilation timeout). Accumulate row-sums first to reduce iterations.
...
7. **Key technique**: Flatten batch*channels, use unit-stride width access for coalesced reads. Avoid 4D index decomposition (division/modulo overhead).
