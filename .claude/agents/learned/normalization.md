...
## Decision Framework for Normalization Tasks

1. **RMSNorm with small feature dim (<=256)**: 2D register tiling (BLOCK_S x FEATURES). All features in registers. Expect 1.5x.
2. **LayerNorm with very large norm dim (4M+)**: Multi-kernel with chunked partial stats. Never single-kernel (insufficient parallelism). Expect 1.2x.
3. **LayerNorm with moderate norm dim**: BLOCK_SIZE must be >= D. Masked positions must be zeroed for variance. Expect 1.0-1.2x.
4. **InstanceNorm**: Sum/sum-of-squares (not Welford). Bandwidth-bound. ~1.0x.
5. **GroupNorm on huge tensors**: 3-kernel (per-channel stats -> per-group reduce -> apply). ~1.0x ceiling.
6. **BatchNorm**: cuDNN is unbeatable. Accept failure. Remember: eval server blocks "BatchNorm" strings.
