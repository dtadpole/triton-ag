...
class UFTTrainer:
    """UFT (Unified Fine-Tuning) trainer for preference learning"""
    ...
    def log_raw_data(self, data: Any, context_vars: Dict[str, Any]):
        """Log raw data"""
        logger.info(
            f"🔍 [UFTTrainer] DuckDB search has found [{len(data)}] rows.\n{data}"
        )
    ...
    def _compute_mini_batch_loss(
        ...
        for i in range(len(advantages)):  # for each generation result in the batch
            ...
            if (
                ...
                raise ValueError(
                    f"❌ [UFTTrainer] Generation completion log probabilities and forward completion log probabilities have different shapes: {generation_completion_log_probs.shape} != {forward_completion_log_probs.shape}"
                )
            if (
                ...
                raise ValueError(
                    f"❌ [UFTTrainer] VLLM completion log probabilities and generation completion log probabilities have different shapes: {vllm_completion_log_probs_i.shape} != {generation_completion_log_probs.shape}"
                )
    ...
    async def train_uft_block(
        ...
        if not success:
            logger.error(
                f"❌ [UFTTrainer] [{block.input_tag}] Failed to execute module config: {self.module_config.get('processor', {}).get('uft_trainer', {})}"
            )
            return
        ...
        try:
            ...
            logger.error(
                f"❌ [UFTTrainer] [{block.input_tag}] Failed to save checkpoint: {e}"
            )
...
def uft_get_trainer(
    ...
    uft_config_file: str = "trainerUFT.yaml",
    module_file: str = "trainer/uft.module.yaml",
):
    """Get a UFT trainer"""
    try:
        engine._update_config(
            EngineConfig.from_yaml(
                engine_config_file, override_yaml_path=uft_config_file
            )
        )
        logger.info(
            f"⚙️ [UFTTrainer] [{prefix_tag}] Engine config [{engine.__class__.__name__}] loaded from [{engine_config_file}]"
        )
    except Exception as e:
        logger.error(
            f"❌ [UFTTrainer] [{prefix_tag}] Engine config [{engine.__class__.__name__}] failed to load: [{type(e)}] {e}"
        )
        raise e
    ...
    try:
        ...
        logger.error(
            f"❌ [UFTTrainer] [{prefix_tag}] Failed to load UFT configuration: {e}"
        )
        raise e
    ...
    try:
        ...
        logger.error(f"❌ [UFTTrainer] [{prefix_tag}] Initialization failed: {e}")
        raise e
...
async def main():
    """Main function for UFT training"""
    parser = argparse.ArgumentParser(description="Train a model using UFTTrainer")
    parser.add_argument("--queue_name", type=str, default="uft.1")
    parser.add_argument("--engine", type=str, default="unsloth")
    parser.add_argument("--engine_config", type=str, default="engineBase.yaml")
    parser.add_argument("--prefix_tag", type=str, default="auto.trainer.uft")
    ...
    parser.add_argument("--output_dir", type=str, default=TRAINER_DIR + "/uft")
    parser.add_argument("--uft_config", type=str, default="trainerUFT.yaml")
    parser.add_argument("--module_file", type=str, default="trainer/uft.module.yaml")
    ...
    trainer = uft_get_trainer(
        engine, args.prefix_tag, args.engine_config, args.uft_config
    )
    uft_block = TrainerBlock(
    ...
    uft_block.input_dir = os.path.expanduser(uft_block.input_dir)
    uft_block.output_dir = os.path.expanduser(uft_block.output_dir)
    ...
    await trainer.train_uft_block(uft_block, callback=callback_func)
