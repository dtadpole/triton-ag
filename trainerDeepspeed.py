...
class DeepspeedTrainer(BaseTrainer):
    ...
    def _checkpoint_exists(self, checkpoint_path: str) -> bool:
        ...
        for i in range(self.world_size):
            if not (checkpoint_path_obj / f"{CHECKPOINT_READY_FILE}.{self.world_size}.{i}").exists():
                return False
        return True
    ...
    def _save_checkpoint(self, step: int, callback: Optional[Callable] = None):
        ...
        (checkpoint_path / f"{CHECKPOINT_READY_FILE}.{self.world_size}.{self.rank}").touch()
    ...
    def _optimization_step(self):
        ...
        # Clip gradients
        grad_norm = clip_grad_norm_(self.engine.module.parameters(), self.config.training.max_grad_norm)
    ...
    async def train_block(
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if (batch_idx + 1) % self.config.training.gradient_accumulation_steps == 0:
                ...
                # Save checkpoint
                if self.status.global_step % self.config.training.save_steps == 0 and self.engine.global_rank == 0:
                    self._save_checkpoint(self.status.global_step, callback=callback)

                # Evaluation
                if eval_dataset and self.status.global_step % self.config.training.eval_steps == 0 and self.engine.global_rank == 0:
