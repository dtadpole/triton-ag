...
class InferenceCustomServer:
    def __init__(self, engine: EngineBase, config_path="inferenceCustom.yaml"):
        self.model_config = from_yaml(config_path).get("model", {})
        self.model_path = os.path.expanduser(
            self.model_config.get("model_path", "~/.trainer")
        )
        ...
        self.engine = engine
        self.base_model = self.engine._base_model()
        self.base_model.eval()
        self.lora_model = self.engine._lora_model()
    ...
    def _enable_api_endpoints(self):
        ...
        async def load_lora_adapter(request: LoadLoraAdapterRequest):
            ...
            if not os.path.exists(lora_path):
                raise HTTPException(
                    status_code=404, detail=f"LoRA checkpoint [{lora_path}] not found"
                )
        ...
        async def unload_lora_adapter(request: UnloadLoraAdapterRequest):
            ...
            if lora_adapter_name not in self.lora_model.peft_config.keys():
                raise HTTPException(
                    status_code=404,
                    detail=f"LoRA checkpoint [{lora_adapter_name}] not found",
                )
        ...
        async def get_logps(request: ProbRequest) -> ProbResponse:
            ...
            if (
                ...
                and model_or_adapter_name not in self.lora_model.peft_config.keys()
            ):
                raise HTTPException(
                    status_code=404,
            ...
            try:
                result = await asyncio.wait_for(
                    result_queue.get(), timeout=self.prob_timeout
                )
                if "error" in result:
                ...
                raise HTTPException(
                    status_code=504, detail="Probability inference timeout"
                )
            return ProbResponse(
    ...
    def _calculate_prob(self, work_items):
        ...
        for item in work_items:
            input_ids_list.append(
                item["input_ids"] + [0] * (max_len - len(item["input_ids"]))
            )
        input_ids = torch.tensor(
            input_ids_list, dtype=torch.long, device=self.engine.device
        )  # (B, L)
        # assert that all the work_items have the same model_name
        assert all(
            item["model_name"] == work_items[0]["model_name"] for item in work_items
        ), "All work items must have the same model_name"
        ...
        per_token_logps = per_token_logps.gather(
            dim=2, index=input_ids[:, 1:].unsqueeze(2)
        ).squeeze(
            2
        )  # (B, L-1)
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    model_item_lengths = [
                        (k, len(v)) for k, v in work_items_by_model.items()
                    ]
                    selected_model_name, selected_length = random.choices(
                        model_item_lengths, weights=[v for k, v in model_item_lengths]
                    )[0]

                    work_items = work_items_by_model[selected_model_name][
                        : self.mini_batch_size
                    ]
                    work_items_by_model[selected_model_name] = work_items_by_model[
                        selected_model_name
                    ][self.mini_batch_size :]
                    try:
                        ...
                        total_tokens = sum(
                            [len(item["input_ids"]) for item in work_items]
                        )
                        token_per_second = (
                            total_tokens / elapsed_time if elapsed_time > 0 else 0.0
                        )
                        logger.info(
                            f"Probability worker [{selected_model_name}] completed [{len(work_items)}] items. [{[len(item['input_ids']) for item in work_items]}] in [{elapsed_time:.2f}s] [{token_per_second:.2f} tokens/s]"
                        )
                        ...
                        for i, item in enumerate(work_items):
                            ...
                            item["result_queue"].put_nowait(result)
                        logger.error(
                            f"Probability worker [{selected_model_name}] task failed. [{[len(item['input_ids']) for item in work_items]}] [{type(e).__name__}] {e}"
                        )
                ...
                logger.error(
                    f"Probability worker task had UNEXPECTED ERROR: [{type(e).__name__}] {e}"
                )
                logger.error(traceback.format_exc())
                continue
            finally:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prefix_tag", type=str, default="auto.inference.custom")
    parser.add_argument("--config", type=str, default="inferenceCustom.yaml")
    ...
    engine = EngineBase.create_engine(
        args.prefix_tag, engine_config, inference_mode=True
    )  # no status for inference
